...
const ChatBot = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const botMessage = { name: "Sam", message: data.response || "No response" };
      setMessages((prevMessages) => [...prevMessages, botMessage]);
    } catch (error) {
      console.error("Error connecting to chatbot API:", error);
      const errorMessage = { name: "Sam", message: "Error: Unable to connect to the server." };
  ...
  return (
    <div className="fixed bottom-0 right-0 md:bottom-5 md:right-5 z-50">
      <button
        className="bg-blue-500 text-white p-3 rounded-full shadow-lg hover:bg-blue-600 focus:outline-none m-4 md:m-0"
      ...
      {isOpen && (
        <div className="fixed inset-0 md:relative md:inset-auto">
          <div className="absolute inset-0 bg-black bg-opacity-50 md:hidden" onClick={toggleChatbox} />
